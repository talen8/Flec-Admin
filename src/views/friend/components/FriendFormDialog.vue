  ...
  <el-dialog v-model="visible" :title="dialogTitle" width="600px" :close-on-click-modal="false">
    <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px">
      ...
      <el-row :gutter="20">
        <el-col :span="9">
          <el-form-item label="友链头像" prop="avatar">
            <ImageUploader ref="avatarUploaderRef" v-model="formData.avatar" upload-type="友情链接A" :max-size="2"
        ...
        <el-col :span="15">
          <el-form-item label="网站截图" prop="screenshot">
            <ImageUploader ref="screenshotUploaderRef" v-model="formData.screenshot" upload-type="友情链接S" :max-size="5"
          ...
          <el-form-item label="友链类型" prop="type_id">
            <el-select v-model="formData.type_id" placeholder="请选择友链类型" style="width: 100%">
              <el-option v-for="type in friendTypeOptions" :key="type.id" :label="type.name" :value="type.id"
                ...
                <span>{{ type.name }}</span>
                <el-tag v-if="!type.is_enabled" size="small" type="info" style="margin-left: 8px">已禁用</el-tag>
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
      ...
      <el-row v-if="isEdit" :gutter="20">
        <el-col :span="12">
          <el-form-item label="失效状态" prop="is_invalid">
            <el-switch v-model="formData.is_invalid" active-text="已失效" inactive-text="正常" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="审核状态" prop="is_pending">
            <el-switch v-model="formData.is_pending" active-text="待审核" inactive-text="已通过" />
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (avatarUploaderRef.value?.getPendingCount()) {
      try {
        await avatarUploaderRef.value.uploadPendingFile()
      } catch (error: any) {
    ...
    // 如果有待上传的截图，先上传
    if (screenshotUploaderRef.value?.getPendingCount()) {
      try {
        await screenshotUploaderRef.value.uploadPendingFile()
      } catch (error: any) {
        submitLoading.value = false
    ...
    if (isEdit.value && props.editFriend) {
      ...
      const createData: CreateFriendRequest = {
        ...
        screenshot: formData.value.screenshot,
        sort: formData.value.sort,
        type_id: formData.value.type_id! // 验证通过后一定不为null
      }
