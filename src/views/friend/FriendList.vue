<template>
  <common-list title="友链管理" :data="friendList" :loading="loading" :total="total" 
    v-model:page="queryParams.page" v-model:page-size="queryParams.page_size"
    create-text="新增友链" @create="handleCreate" @refresh="fetchFriends"
    ...
    <el-table-column label="友链名称" min-width="150">
      <template #default="{ row }">
        <span>{{ row.name }}</span>
        <el-tag v-if="row.is_pending" type="info" size="small" style="margin-left: 8px">待审核</el-tag>
